    ...
    for j in range(int((height - size * RepetitionRate) / (size * (1 - RepetitionRate)))):
        for i in range(int((width - size * RepetitionRate) / (size * (1 - RepetitionRate)))):
            if (j == height // size):
                ...
                if (len(data.shape) == 2):
                    cur_image = data[
                                int(i * size * (1 - RepetitionRate)): int(i * size * (1 - RepetitionRate)) + size,
                                int(j * size * (1 - RepetitionRate)): int(j * size * (1 - RepetitionRate)) + size]
                #  如果图像是多波段
                else:
                    cur_image = data[:,
                                int(i * size * (1 - RepetitionRate)): int(i * size * (1 - RepetitionRate)) + size,
                                int(j * size * (1 - RepetitionRate)): int(j * size * (1 - RepetitionRate)) + size]
                lon = minx + int(xres * size * j * (1 - RepetitionRate))
                lat = maxy + int(yres * (i * size * (1 - RepetitionRate)))
...
if __name__ == "__main__":
    ...
    ClipMydatasetTif(r"D:\SummerSentinelData\StretchData\15Bands\StretchImages\20200904_T50SKE_area3.tif", 0.5,
                     r'D:\SummerSentinelData\StretchData\15Bands\TrainingData20211014\Image\{}_{}.tif', 256)

    ClipMydatasetTif(r"D:\SummerSentinelData\StretchData\MaskData\20200904_T50SKE_area_3_Mask.tif", 0.5,
                     r'D:\SummerSentinelData\StretchData\15Bands\TrainingData20211014\Label\{}_{}.tif', 256)
